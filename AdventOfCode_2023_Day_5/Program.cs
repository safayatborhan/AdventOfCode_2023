...
public class Progam
{
    public static void Main(string[] args)
    {
        ...
        //string input = @"seeds: 41218238 421491713 1255413673 350530906 944138913 251104806 481818804 233571979 2906248740 266447632 3454130719 50644329 1920342932 127779721 2109326496 538709762 3579244700 267233350 4173137165 60179884
        ...
        var result = GetMinimumLocationFromSeedsRange(input).GetAwaiter().GetResult();
        Console.WriteLine(result);
    }

    private static long GetMinimumLocation(string input)
    {
        const string seedsToSoil = "seed-to-soil", soilToFertilizer = "soil-to-fertilizer", fertilizerToWater = "fertilizer-to-water", waterToLight = "water-to-light", lightToTemperature = "light-to-temperature", temperatureToHumidity = "temperature-to-humidity", humidityToLocation = "humidity-to-location";

        var mappings = GetMappingDictionaryWithRange(input);
        var mappingSets = mappings.mappingSets;
        var seeds = mappings.seeds;

        var soils = GetSourceDestinationSet(mappingSets[seedsToSoil], seeds);

        var fertilizers = GetSourceDestinationSet(mappingSets[soilToFertilizer], soils);

        var waters = GetSourceDestinationSet(mappingSets[fertilizerToWater], fertilizers);

        var lights = GetSourceDestinationSet(mappingSets[waterToLight], waters);

        var temperatures = GetSourceDestinationSet(mappingSets[lightToTemperature], lights);

        var humidities = GetSourceDestinationSet(mappingSets[temperatureToHumidity], temperatures);

        var locations = GetSourceDestinationSet(mappingSets[humidityToLocation], humidities);

        return locations.Min();
    }

    private static async Task<long> GetMinimumLocationFromSeedsRange(string input)
    {
        const string seedsToSoil = "seed-to-soil", soilToFertilizer = "soil-to-fertilizer", fertilizerToWater = "fertilizer-to-water", waterToLight = "water-to-light", lightToTemperature = "light-to-temperature", temperatureToHumidity = "temperature-to-humidity", humidityToLocation = "humidity-to-location";

        var mappings = GetMappingDictionary(input);
        var mappingSets = mappings.mappingSets;
        var seeds = mappings.seeds;
        var channel = Channel.CreateUnbounded<long>();

        var seedsRange = new List<long>();
        var locationsRange = new List<long>();

        var writer = channel.Writer;
        for (int i = 0; i < seeds.Count; i += 2)
        {
            await writer.WriteAsync(seeds[i]);
        ...
        var seedsReader = channel.Reader;

        var soils = await GetSourceDestinationSetFromRange(mappingSets[seedsToSoil], seedsReader);

        var fertilizers = await GetSourceDestinationSetFromRange(mappingSets[soilToFertilizer], soils);

        var waters = await GetSourceDestinationSetFromRange(mappingSets[fertilizerToWater], fertilizers);

        var lights = await GetSourceDestinationSetFromRange(mappingSets[waterToLight], waters);

        var temperatures = await GetSourceDestinationSetFromRange(mappingSets[lightToTemperature], lights);

        var humidities = await GetSourceDestinationSetFromRange(mappingSets[temperatureToHumidity], temperatures);

        var locations = await GetSourceDestinationSetFromRange(mappingSets[humidityToLocation], humidities);

        var readerTask = Task.Factory.StartNew(async () =>
        {
            while (!locations.Completion.IsCompleted)
            {
                var location = await locations.ReadAsync();

                locationsRange.Add(location);

            }
        });

        return locationsRange.Min();
    }

    private static async Task<ChannelReader<long>> GetSourceDestinationSetFromRange(List<List<long>> mappingSet, ChannelReader<long> reader)
    {
        var destinationValues = new List<long>();
        var sourceValuesThatMatched = new List<long>();

        var mappingSetCount = mappingSet.Count;
        ...
        var readerTask = Task.Factory.StartNew(async () =>
        {
            ...
            {
                var sourceValue = await reader.ReadAsync();
                var isAlreadyFound = false;
                for (int index = 0; index < mappingSetCount; index++)
                {
                    if (sourceValue >= mappingSet[index][1] && sourceValue <= (mappingSet[index][1] + mappingSet[index][2]))
                    {
                        var destinationValue = mappingSet[index][0] + (sourceValue - mappingSet[index][1]);
                        await writer.WriteAsync(destinationValue);
                        if (!isAlreadyFound)
                        {
                            isAlreadyFound = true;
                            break;
                ...
                if(!isAlreadyFound)
                {
                    await writer.WriteAsync(sourceValue);
                }
            }
        });
        ...
        var destinationReader = channel.Reader;

        return destinationReader;
    }

    private static List<long> GetSourceDestinationSet(List<List<long>> mappingSet, List<long> sourceValues)
    {
        var destinationValues = new List<long>();
        ...
        foreach (var sourceValue in sourceValues)
        {
            ...
            {
                if (mapping[1] > sourceValue)
                {
                    continue;
                }
                else
                {
                    if (mapping[1] + mapping[2] >= sourceValue)
                    {
                        isFound = true;
                        destinationValue = mapping[0] + (sourceValue - mapping[1]);
                        break;
                    }
                }
            }
    ...
    private static (List<long> seeds, Dictionary<string, List<List<long>>> mappingSets) GetMappingDictionary(string input)
    {
        var mappingSets = new Dictionary<string, List<List<long>>>();
        var seedsSet = new Dictionary<string, List<long>>();
        var mapList = new List<List<long>>();
        var mapListElements = new List<long>();
        var mappingBlocks = input.Split("\r\n\r\n");
        var count = 0;

        foreach (var mappingBlock in mappingBlocks)
        {
            count++;
            var mapKeyValueList = mappingBlock.Split(":");
            var mappingValues = mapKeyValueList[1].Split("\r\n");
            ...
            foreach (var mappingValue in mappingValues)
            {
                ...
                {
                    long sum = 0;
                    foreach (var mappingValueChar in mappingValue.Trim() + " ")
                    {
                        if (char.IsDigit(mappingValueChar))
                        {
                            sum = (sum * 10) + long.Parse(mappingValueChar.ToString());
                        }
                        else
                        {
                            mapListElements.Add(sum);
                            sum = 0;
                        }
                    }

                    mapList.Add(mapListElements);
                    mapListElements = new List<long>();
                }
            }

            if(count == 1)
            {
                seedsSet.Add(mapKeyValueList[0].Trim().Replace(" map", string.Empty), mapList[0]);
            }
            else
            {
                mappingSets.Add(mapKeyValueList[0].Trim().Replace(" map", string.Empty), mapList);
            }
            mapList = new List<List<long>>();
        }

        return (seedsSet["seeds"], mappingSets);
    }

    private static (List<long> seeds, Dictionary<string, List<List<long>>> mappingSets) GetMappingDictionaryWithRange(string input)
    {
        var mappingSets = new Dictionary<string, List<List<long>>>();
        var seedsSet = new Dictionary<string, List<long>>();
        var mapList = new List<List<long>>();
        ...
        foreach (var mappingBlock in mappingBlocks)
        {
            ...
            {
                //var sortedList = mapList.OrderBy(x => x[1]).ToList();
                mappingSets.Add(mapKeyValueList[0].Trim().Replace(" map", string.Empty), mapList);
            }
            mapList = new List<List<long>>();
        }
